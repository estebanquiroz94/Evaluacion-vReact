...
class ProductCatalogComponent extends Component {
  ...
  performGetProducts() {
    ...
    fetch("http://localhost:3008/api/products/getAllProducts", {
      method: 'get',
      headers: headers
      ...
      .then(responseJson => {
        if (responseJson.error) {
          alert('error');
            ...
            product.name = element.nameProduct;
              product.image = `${element.image.substring(element.image.lastIndexOf('/') + 1)}`;
            product.price = element.price;
            product.stock = element.unitsAvailable;
